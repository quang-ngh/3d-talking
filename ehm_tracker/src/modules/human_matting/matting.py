...
class StyleMatteModel(torch.nn.Module):
    def __init__(self, device='cpu', ckpt_fp=''):
        super().__init__()
        self.device = device
        self.ckpt_path = ckpt_fp
        self.init_models()
    ...
    def forward(self, input_image, return_type='matting', background_rgb=1.0):
        ...
        mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).expand(input_image.shape[0], -1, -1, -1).to(input_image.device)
        std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).expand(input_image.shape[0], -1, -1, -1).to(input_image.device)
