...
#         # if key.__class__==torch.Tensor:
#             npy_dict[key] = tensor_dict[key].cpu().numpy()
#         # else:
#         #     npy_dict[key] = tensor_dict[key]
#     print(npy_dict)
#     return npy_dict

def perspective_projection(points, rotation, translation,camera_intrics):
    """
    ...
    K=torch.tensor([camera_intrics], device=points.device,dtype=torch.float)
...
def estimate_translation_np(S, joints_2d, joints_conf, camera_intrics):
    ...
    # focal length
    f = np.array([camera_intrics[0][0], camera_intrics[1][1]])
    # optical center
    center = np.array([camera_intrics[0][2], camera_intrics[1][2]])
