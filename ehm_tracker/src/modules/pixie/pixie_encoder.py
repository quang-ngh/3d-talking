...
from .utils.util import perspective_projection, estimate_translation_np
from .utils import rotation_converter as converter
from .utils import tensor_cropper
from .utils.config import cfg
from ...utils.rprint import rlog as log
from ..smplx.SMPLX_1 import SMPLX

# from test4 import


class PixieEncoder(nn.Module):
    def __init__(self, config=None, ckpt_fp=''):
        super().__init__()
        if config is None:
            self.cfg = cfg
        else:
        ...
        if os.path.exists(ckpt_fp):
            self.cfg.pretrained_modelpath = ckpt_fp
    ...
    def _create_model(self):
        ...
        for key in self.cfg.network.regressor.keys():
            ...
            if self.cfg.network.regressor.get(key).type == 'mlp':
                self.Regressor[key] = MLP(channels=channels)
        ...
        for key in self.cfg.network.extractor.keys():
            ...
            if self.cfg.network.extractor.get(key).type == 'mlp':
                self.Extractor[key] = MLP(channels=channels)
        ...
        self.smplx = SMPLX('assets/SMPLX', n_shape=200, n_exp=50)
        ...
        if os.path.exists(model_path):
            ...
            log(f'pixie trained model path: {model_path} does not exist!')
            exit()
        # eval mode
        for module in [self.Encoder, self.Regressor, self.Extractor, self.Moderator]:
    ...
    @torch.no_grad()
    def forward(self, image, image_hd, threthold=True, keep_local=True, copy_and_paste=False, body_only=False):
        ...
        # encode feature
        feature[key] = {}
        feature[key][part] = self.Encoder[part](image)
