...
class Cropper(object):
    ...
    def crop_single_image(self, obj, fast_ret=False):
        direction = self.crop_cfg.get('direction', 'large-small')
        ...
        ret_dct = crop_image(
            img_rgb,  # ndarray
            pts,  # 106x2 or Nx2
            dsize=self.crop_cfg.get('dsize', 512),
            scale=self.crop_cfg.get('scale', 2.3),
            vy_ratio=self.crop_cfg.get('vy_ratio', -0.15),
        )
        if fast_ret:
            return ret_dct
        ...
        ret_dct['pt_crop_256x256'] = ret_dct['pt_crop'] * 256 / self.crop_cfg.get('dsize', 512)
