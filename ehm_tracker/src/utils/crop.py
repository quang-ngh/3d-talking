...
def parse_bbox_from_landmark_lite(pts, scale=1.5, need_square=True):
    
    lt_pt = np.min(pts, axis=0)
    rb_pt = np.max(pts, axis=0)
    center = (lt_pt + rb_pt) / 2
    ...
    # calculate the vertex positions before rotation
    bbox = np.array([cx-w/2, cy-h/2, cx+w/2, cy+h/2], dtype=DTYPE)  # left, top, right, bottom
