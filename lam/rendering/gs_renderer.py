...
from lam.rendering.flame_model.flame import FlameHeadSubdivided
from lam.model.transformer import TransformerDecoder
from pytorch3d.transforms import matrix_to_quaternion
from lam.rendering.utils.typing import *
from lam.rendering.utils.utils import trunc_exp, MLP
from lam.rendering.gaussian_model import GaussianModel
...
import lam.rendering.utils.mesh_utils as mesh_utils
from lam.rendering.utils.point_utils import depth_to_normal
    ...
    def __init__(self, query_dim, context_dim, num_heads, mlp=False, decode_with_extra_info=None):
        ...
        if decode_with_extra_info is not None:
            if decode_with_extra_info["type"] == "dinov2p14_feat":
                ...
                from lam.encoder.dinov2_fusion_wrapper import Dinov2FusionWrapper
                self.encoder = Dinov2FusionWrapper(model_name='dinov2_vits14_reg', modulation_dim=None, freeze=False, encoder_feat_dim=384)
                ...
                try:
                    from lam.encoder.xunet_wrapper import XnetWrapper
                    self.encoder = XnetWrapper(model_name='resnet18', freeze=False, encoder_feat_dim=64)
                except ImportError:
                    raise ImportError("decoder_resnet18_feat requires lam.encoder.xunet_wrapper (not included in base lam)")
...
def test_head():
    ...
    try:
        ...
        VideoHeadDataset = None
    if VideoHeadDataset is None:
        print("VideoHeadDataset not available; skip test_head()")
        return
